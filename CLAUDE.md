...
- `work` -- **unified lifecycle entry point**; detects current state and routes automatically: `work` alone starts or resumes, `work-end` closes the branch, `work-pause` saves state and returns to main; `work epic #N` sets up single-repo epic iteration from a parent issue with child issues, `work next` advances to the next child issue in the epic
- `lifecycle.py` -- lifecycle state machine (`project/lifecycle.py`); owns all state transitions via a data-driven table; entry points fire events, the machine determines effects; 11 states (idle, scaffolded, active, transitioning, paused, closing:review/verified/promoted/pushed/merged/stamped); three-phase protocol (transition, effects, commit_transition); pre-push hook (`project/pre_push_hook.py`) enforces closing gates
- `work-start` -- unified entry point for all work; reads `META_STATE` from ctx.py for state-based routing; creates `issue-NNN-<slug>` branches in both repos atomically; scaffolds `.meta` + `JOURNAL.md` with SHA baseline, design routing, and `state: scaffolded`; runs platform coherence, protocols, IntelliJ pre-checks
- `work-end` -- closes the current branch; workspace-promotes and project-promotes artifacts per routing config; merges `design/JOURNAL.md` into ARC42STORIES.MD with three-way diff preview; posts specs to GitHub issue; closes issue; marks branch with `design/EPIC-CLOSED.md`; returns both repos to main
