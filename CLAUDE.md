...
- `work` -- **unified lifecycle entry point**; five commands: `work` (start or resume), `work end` (close), `work pause` (save state), `work next` (advance to next issue in `.plan` queue), `work find` (discover and populate queue). Commands chain bidirectionally: forward when nothing to do (`continue→next→end→find`), backward when not ready (`find→next→continue`). Chaining logic is in Python (`work_chain.py`), not LLM. Epics auto-detected from GitHub — no separate epic command
- `lifecycle.py` -- lifecycle state machine (`project/lifecycle.py`); owns all state transitions via a data-driven table; entry points fire events, the machine determines effects; 12 states (idle, scaffolded, active, transitioning, paused, drained, closing:review/verified/promoted/pushed/merged/stamped); `drained` = queue empty, ceremony done, plan persists, re-activatable via `work find`; three-phase protocol (transition, effects, commit_transition); pre-push hook (`project/pre_push_hook.py`) enforces closing gates
- `work_chain.py` -- bidirectional chaining engine (`project/work_chain.py`); evaluates `continue`/`next`/`end`/`find` commands and returns deterministic directives (`proceed`, `chain_to_next`, `guard_continue`, etc.); the LLM reads directives, never makes routing decisions
- `work start` -- unified entry point for all work; accepts 1..n issue numbers OR free text; auto-detects epics and builds a `.plan` queue; creates `issue-NNN-<slug>` branches in both repos atomically; scaffolds unified `.plan` (## State + ## Queue) and `JOURNAL.md` with SHA baseline, design routing, and `state: scaffolded`; runs platform coherence, protocols, IntelliJ pre-checks
- `work end` -- closes the current branch or main work; works on any branch or main. On branches: workspace-promotes and project-promotes artifacts per routing config; merges `JOURNAL.md` into ARC42STORIES.MD with three-way diff preview; posts specs to GitHub issue; closes issue; stamps branch; returns both repos to main. On main: same ceremony minus merge/stamp/rebase/squash; sets state to `drained`. In a slot, runs the full sequence (review, promote, squash, push, merge to canonical, stamp, archive)
...
- `write-blog` -- living project diary; captures decisions, pivots, and discoveries in diary voice as they happen; consumes write-content for content type and writing guidance
