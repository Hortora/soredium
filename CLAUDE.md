...
- `work` -- **unified lifecycle entry point**; four commands: `work` (start or resume), `work-end` (close), `work-pause` (save state), `work next` (advance to next issue in `.plan` queue). Epics auto-detected from GitHub — no separate epic command
...
- `work-start` -- unified entry point for all work; accepts 1..n issue numbers OR free text; auto-detects epics and builds a `.plan` queue; creates `issue-NNN-<slug>` branches in both repos atomically; scaffolds `.meta` + `JOURNAL.md` + `.plan` with SHA baseline, design routing, and `state: scaffolded`; runs platform coherence, protocols, IntelliJ pre-checks
- `work-end` -- closes the current branch; one command regardless of context (branch or slot). Workspace-promotes and project-promotes artifacts per routing config; merges `design/JOURNAL.md` into ARC42STORIES.MD with three-way diff preview; posts specs to GitHub issue; closes issue; marks branch with `design/EPIC-CLOSED.md`; returns both repos to main. In a slot, runs the full sequence (review, promote, squash, push, merge to original, stamp, archive)
