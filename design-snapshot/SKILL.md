...
**Invoked by:** User directly: "create a design snapshot", "snapshot where we are", "document our progress"; [`handover`] — offered as part of the session wrap checklist (off by default, user must tick it explicitly); [`write-content`] — offered after writing a major milestone entry (not automatic; user is offered the choice)
