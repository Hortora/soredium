...
Epics expand their children inline. Nested epics are supported. Epics
with 5+ children get batch planning — grouping by domain affinity and
dependencies, with batch boundaries as safe exit points.

To advance to the next issue:

```
work next
```

This checks off the current issue, moves the active marker, and ticks
