...
| `invalidation_status` | `pending` \| `resolved` | Tracks the state of a structural fix described in `invalidation_triggers`. Set to `pending` via REVISE when a trigger describes a fix that hasn't been found yet. Set to `resolved` (then RETIRE the entry) when the fix is confirmed. Surfaced prominently during harvest REVIEW so flagged entries get focused follow-up. |
