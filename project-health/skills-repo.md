...
- [ ] Every user-invokable skill has a `commands/<skill-name>.md` (skills with `slash-command: false` are exempt)
...
- [ ] Every user-invokable skill has a `commands/<skill-name>.md` slash command file
...
| Flagging missing slash command for auto-triggered skills | Skills with `slash-command: false` are deliberately excluded | Only flag missing `commands/<name>.md` for user-invokable skills |
