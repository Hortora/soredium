...
for a step. The dispatch loop emits FALLBACK_TRIGGERED before falling
back. After handling the step manually, call the orchestrator again to
continue the sequence.

### Fallback: Review (Step 2)
...
out findings just written by Steps 2.1 and 2.2.
...
**Re-review after fixes:** When "Fix" creates new commits, re-run
code-review on those commits only. Branch-audit does not re-run.
...
After success: fire `review_pass` then `promote_pass` lifecycle transitions.

**4.1b Trajectory capture**

Non-blocking. Draft trajectory notes, propose enrichment updates,
persist on user confirmation via enrichment.py.
...
Post-rebase re-review: code-review only on conflict resolution diff.

**4.3 Squash analysis**

For each repo: classify commits and write `.squash-plan-<repo>.json`.

Slot mode: write `.phase-a-complete` marker after squash:
```bash
...
Main mode: push directly:
```bash
git -C $PROJ push origin main
git -C $WS push origin main
```

After land: fire `push_pass`, `merge_pass`, `stamp_pass` lifecycle
transitions in rapid succession.

### Fallback: Verify (Step 5)

```bash
python3 work-end/verify_slot_close.py <PROJ> branch=<BRANCH> workspace=<WS> [covers=<CSV>] [slot_dir=<SLOT_PATH>]
```

**Hard gate:** `VERIFIED=no` blocks close. Present failures, offer recovery.
...
Main mode: Keep `.plan`, remove only JOURNAL.md and transient files.

**6.3 Stack cleanup:** Remove closed branch from `.pause-stack`.

**6.4 ARC42 stale scan:** Scan ARC42STORIES.MD for stale statuses.

**6.5 Session rename:** Suggest a descriptive session name.

**6.6 Garden retrieval feedback:** Record which garden entries were useful
via gardenFeedback MCP calls. Non-blocking.
...
**6.8 Notes:** Surface `$WORKSPACE/.notes/NOTES.md` recent section,
offer to append, commit to orphan notes branch.
</FALLBACK>
